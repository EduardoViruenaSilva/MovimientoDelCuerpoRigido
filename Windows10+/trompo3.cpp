...
double q(double k)
{
    return exp( -M_PI * K(sqrt(1.0-sqr(k))) / K(k));
}
...
double kp(double k)
{
   return sqrt(1.0-sqr(k));
}


double cn(double u, double k)
{
    return sqrt(kp(k)/k) * H(u+K(k), k) / Te(u,k);
}


double sn(double u, double k)
{
    return H(u,k) / (sqrt(k) * Te(u,k));
}

double dn(double u, double k)
{
    return sqrt(kp(k)) * Te(u+K(k),k)/Te(u,k);
}
...
class List : public vector<GLuint> {
      public:
         List(void) : vector<GLuint> () {};
};
           ...
           const Vector3D& c, double r)
{
    ...
    Vector3D u= (1.0 - 3.0 * r * (1.0 + sqrt(5.0)) / 2.0) * a;
...
double f(double z)
{
   return sn(z,k2) - sqrt((e1-e0)/(e1-e3));
}
...
cerr << "e3= " << e3 << endl;
   ...
   k1= sqrt( ((e0-e3)*(e1-e2)) / ((e1-e0)*(e2-e3)) ); 

cerr << "k1= " << k1 << endl;

   k2= sqrt( ((e1-e3)*(e2-e0)) / ((e1-e0)*(e2-e3)) ); 
   ...
   for (int i=0; i<N; i++)
      {
       ...
       dcomplex c=  cn(z,k2)/sqrt(1.0-sqr(sn(z,k2))*sqr(dn(u,k1))) *
       ...
       if (abs( h%r - s ) > 1E-12)
           cerr << "ERROR - vectors are not orthogonal\n";
...
{
    double a= 5;
    cerr << "Dar divisor de Pi (numero real mayor que 3):\n";
    cin >> a;

    double e= -0.3;
    cerr << "Dar e0 (numero real menor que cos(Pi/a-2Pi/3)="
        <<  cos(M_PI/a - 2.0*M_PI/3.0)
        <<  " y mayor que cos(Pi/a+2Pi/3)= "
        <<  cos(M_PI/a + 2.0*M_PI/3.0) <<  endl;
    cin >> e;
